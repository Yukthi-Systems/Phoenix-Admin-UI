...
const EditAttachmentPolicy = () => {
  ...
  const { data, isLoading, isError, error } = useGetAttachmentPolicyById({
    organization_id,
    domain_name,
    policy_id,
  });
  ...
  const { mutate, isPending } = useEditAttachmentPolicy();
  ...
  const onSubmit = (formData) => {
    ...
    mutate(
      { organization_id, domain_name, policy_id, data },
      {
        onSuccess: () => {
  ...
  if (isError && isServerError) {
    return (
      <DataFechError content="Error while loading attachment policy details." />
    );
  }

  if (isLoading) {
    return (
      <DataLoading content="Loading attachment policy details for editing." />
    );
  }
  ...
  return (
    <StepperFormLayout
      breadcrumbItems={[
        ...
        { name: policy?.policy_name || "Edit Attachment Policy" },
      ]}
      steps={STEPS}
      ...
      isPending={isPending}
      submitLabel="Update Attachment Policy"
