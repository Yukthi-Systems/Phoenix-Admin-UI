...
import { useEditPolicyRule, useGetPolicyRule } from "@/hooks/usePolicyRules";
...
function EditPolicyRules() {
  ...
  const { mutate, isPending } = useEditPolicyRule();
  const toast = useToastify();
  ...
  const onSubmit = (formData) => {
    ...
    mutate(
      { organization_id, rule_id: policy_rule_id, data },
      {
        onSuccess: () => {
          toast("success", "Successfully updated policy rule");
          navigate(-1);
        },
        onError: (error) => {
          const message =
  ...
  if (!permissions.includes("policy:rule:edit"))
    return (
      <AccessDenied content="Don't have the access to edit policy rules." />
    );

  if (isError)
    return <DataFechError content="Policy rule details getting error...!" />;
    ...
    <>
      <div className="w-full h-full px-2 overflow-hidden">
        ...
        <div className="h-[calc(100vh-150px)] flex-1 overflow-y-auto bg-card shadow-lg rounded-md no-scrollbar border border-border">
          {isLoading ? (
            <DataLoading content="Policy rule details loading...!" />
          ) : (
            <form
              onSubmit={handleSubmit(onSubmit)}
              className="mx-auto rounded-xl px-5 py-2 space-y-5 text-left"
            >
              <FormHeader text="Edit Policy Rules" />
...
export default EditPolicyRules;
