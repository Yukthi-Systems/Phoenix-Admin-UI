...
const ListFiltersPolicy = () => {
  ...
  const { data, isLoading, isError, error, refetch } = useFiltersPolicy(
    organization_id,
    domainName,
  ...
  const fetchFiltersPolicyForExport = async ({
  ...
  const {
    ...
    isItemSelected,
    removeFromSelection,
  } = useBulkSelection(filters, "policy_id", "policy_name");
  ...
  const {
    ...
    isImportAvailable,
  } = useBulkImport("filters_policies", async (policyData) => {
  ...
  const {
    ...
    "filtersPolicies",
    fetchFiltersPolicyForExport,
    { domain_name: domainName },
    FIELD_MAPPINGS.filtersPolicies,
  );
  ...
  const getDetails = async (id) => {
    try {
      const data = await getFiltersPolicyEntry(organization_id, id);
      setFullData(data);
    } catch (err) {
  ...
  const handleNext = async (targetDomain) => {
    navigate(
      { pathname: `/policies/filters/copy/${targetDomain}` },
      {
        state: {
          fullData: {
            ...fullData,
  ...
  const handleAddFiltersPolicy = () => {
    navigate(`/policies/filters/add/${domainName}`);
  };

  const handleImportCompleteWithRefresh = (results) => {
    handleImportComplete(results);
    const ActionLog = {
      action_type: "import_filters_policy",
      message: `Imported filters policies via bulk import`,
      payload: {
        ...results,
  ...
  const handleBulkDelete = async (itemId, itemName = "Unknown Policy") => {
    return new Promise((resolve, reject) => {
      mutate(
        { org_id: organization_id, policy_id: itemId, policy_name: itemName },
        {
          onSuccess: () => {
            resolve();
          },
  ...
  const columns = useMemo(() => {
    const baseColumns = [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Link
            className="main-col"
            to={`/policies/filters/${row?.original?.policy_id}`}
          >
    ...
    if (
      ...
      permissions.includes("policy:filters:delete")
    ) {
      baseColumns.push({
        id: "actions",
        header: "",
        cell: ({ row }) => {
          ...
          if (permissions.includes("policy:filters:edit")) {
            actions.push({
              ...
              variant: "default",
              onClick: () =>
                navigate(`/policies/filters/edit/${row?.original?.policy_id}`),
              tooltip: "Edit Policy",
            });
          }
          ...
          if (permissions.includes("policy:filters:delete")) {
            if (actions.length > 0) {
  ...
  const multipleSelectOption = useMemo(() => {
    ...
    if (permissions.includes("policy:filters:delete")) {
      options.push({
        label: "Delete",
        description: "Delete filters policy",
    ...
    if (permissions.includes("policy:filters:edit")) {
      options.push({
        label: "Copy",
        description: "Copy filters policy between domain",
  ...
  const createOptions = useMemo(() => {
    ...
    if (permissions.includes("policy:filters:create")) {
      options.push({
        ...
        description: "Create one filters policy",
        icon: <Plus className="h-4 w-4" />,
        onClick: handleAddFiltersPolicy,
      });
    }

    if (
      permissions.includes("policy:filters:create") &&
      isImportAvailable &&
      domainName
    ) {
      options.push({
  ...
  if (!permissions.includes("policy:filters:view")) {
    return (
      <AccessDenied content="Don't have the access to list the filters policies." />
    );
  }

  if (isError && isServerError) {
    return <DataFechError content="Error loading filters policies...!" />;
  }
    ...
    <>
      <div className="flex h-full w-full flex-col px-2">
        ...
        <div className="mb-2.5 flex w-full items-center justify-between gap-6">
          <Breadcrumbs items={[{ name: "Filters Policies" }]} />

          <div className="flex min-w-xl flex-1 gap-2">
            <SearchBar
              placeholder="Search filters policies..."
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            {selectedCount > 0 && domainName && (
              <MultiDelete
                permission="policy:filters:delete"
        ...
        {!domainName ? (
          ...
          <>
            {filters.length !== 0 || isLoading ? (
              ...
              <NoDataFound content="No filters policies found for this domain" />
            )}
          </>
        )}
      </div>
      ...
      <BulkDeleteModal
        ...
        title="Bulk Delete Filters Policies"
        description="Are you sure you want to delete the selected filters policies?"
        itemName="policy"
      />
      ...
      <ExportModal
        ...
        title="Export Filters Policies"
        description="Export all filters policies to Excel format. This may take a few minutes for large datasets."
      />
...
export default ListFiltersPolicy;
