...
const MemoryMetricsChart = ({ data, isLoading }) => {
  ...
  const { chartData, layout, config } = useMemo(() => {
    ...
    const memTotalValues = metrics.map((m) =>
      parseFloat((m.avg_mem_total_mb / 1024)?.toFixed(1)),
    );
    const memUsedValues = metrics.map((m) =>
      parseFloat((m.avg_mem_used_mb / 1024)?.toFixed(2)),
    );
    const memAvailableValues = metrics.map((m) =>
      parseFloat((m.avg_mem_available_mb / 1024)?.toFixed(1)),
    );
    const memPercentValues = metrics.map((m) =>
      parseFloat(m.avg_mem_percent?.toFixed(2)),
    );

    // Create traces
    const traces = [];

    // Memory Total Reference Line
    if (selectedMetrics.memTotal) {
      traces.push({
        x: timestamps,
        y: memTotalValues,
        type: "scatter",
    ...
    // Memory Used Area Chart
    if (selectedMetrics.memUsed) {
      traces.push({
        x: timestamps,
        y: memUsedValues,
        type: "scatter",
    ...
    // Memory Available Area Chart
    if (selectedMetrics.memAvailable) {
      traces.push({
        x: timestamps,
        y: memAvailableValues,
        type: "scatter",
        mode: "lines",
        fill: "tozeroy",
        ...
        hovertemplate:
          "<b>🟢 Available Memory</b><br>" +
          "Available: %{y:.1f} GB<br>" +
          "<extra></extra>",
        hoverlabel: {
          bgcolor: isDarkMode
            ? "rgba(23, 23, 23, 0.95)"
    ...
    // Memory threshold line (50% usage warning)
    if (memPercentValues.some((val) => val > 10)) {
      traces.push({
        x: timestamps,
    ...
    const plotLayout = {
      ...
      yaxis: {
        ...
        showgrid: false,
        ticksuffix: " GB",
        domain: [0, Math.max(50, Math.max(...memTotalValues) + 1)],
        showspikes: true,
        spikethickness: 1,
      ...
      yaxis2: {
        title: {
          text: "Usage (%)",
          font: { color: "hsl(38, 92%, 50%)" },
        },
        overlaying: "y",
        side: "left",
        tickfont: { color: "hsl(38, 92%, 50%)" },
        showgrid: false,
        zeroline: false,
        ticksuffix: "%",
        width: 40,
      ...
      modeBarButtonsToAdd: [
        {
          ...
          click: function (gd) {
            const csvContent =
              ...
              "Time,Total Memory (GB),Used Memory (GB),Available Memory (GB),Usage (%)\n" +
              metrics
                .map(
                  (m) =>
                    `${new Date(m.interval_start).toISOString()},${(m.avg_mem_total_mb / 1024).toFixed(1)},${(m.avg_mem_used_mb / 1024).toFixed(2)},${(m.avg_mem_available_mb / 1024).toFixed(1)},${m.avg_mem_percent.toFixed(2)}`,
                )
                .join("\n");
            ...
            link.setAttribute("download", "memory_metrics.csv");
    ...
    return (
      <div className="bg-card border rounded-lg p-6 shadow-sm">
        <div className="flex flex-col items-center justify-center h-80 text-center">
          <MemoryStick className="w-12 h-12 text-muted-foreground/30 mb-3" />
    ...
    <div className="bg-card border rounded-lg p-6 shadow-sm">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Memory utilization and availability (low usage is normal)
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 pt-3 border-t border-border/50">
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>{data.total_records} total records • Memory in GB</span>
...
export default MemoryMetricsChart;
