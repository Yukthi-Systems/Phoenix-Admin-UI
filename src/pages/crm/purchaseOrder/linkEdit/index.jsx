...
import { useEditCRMPOLink } from "@/hooks/useCRMPO";
...
function CRMLinkEdit() {
  const { po_id, assignment_id } = useParams();
  const location = useLocation();
  const { data: EditData } = location.state;
  ...
  const { mutate, isPending } = useEditCRMPOLink();
  ...
  const onSubmit = (formData) => {
    ...
    data.details.updated_at = toLocalISOString(new Date());
    data.details.domain_dropdown = selectedDomains;

    mutate(
      { organization_id, po_id, service_code: sercode, assignment_id, data },
      {
        onSuccess: () => {
          toast("success", "Successfully updated CRM purchase order link");
          navigate(-1);
        },
        onError: (error) => {
          const message =
  ...
  useEffect(() => {
    if (EditData) {
      reset({
        details: {
          created_at: EditData?.service_details?.created_at || "",
          created_by: EditData?.service_details?.created_by || "",
          description: EditData?.service_details?.description || "",
          service_type: EditData?.service_details?.service_type || "",
          renewal_status: EditData?.service_details?.renewal_status || "",
          start_date: EditData?.service_details?.start_date || "",
          expiry_date: EditData?.service_details?.expiry_date || "",
          branch: EditData?.service_details?.branch || "",
          contact_person: EditData?.service_details?.contact_person || "",
        },
        notes: EditData?.notes || "",
        service_code: EditData?.service_code || "",
      });
      setSelectedDomains(EditData?.service_details?.domain_dropdown || []);
    }
  }, [EditData, assignment_id, po_id, reset]);

  if (!permissions.includes("crm:purchase_order:edit"))
    return <AccessDenied content="Don't have the access to edit link po." />;
    ...
    <div className="w-full h-full px-2 overflow-hidden">
      ...
      <div className="h-[calc(100vh-140px)]  flex flex-col bg-card shadow-lg rounded-md border border-border overflow-hidden">
        ...
        <div className="border-border bg-card border-b px-6 py-6 text-left flex-shrink-0">
          <FormHeader text="Edit CRM purchase order service link" />
        </div>
        ...
        >
          ...
          <div className="flex-shrink-0 from-muted/30 bg-gradient-to-r to-transparent p-6 flex justify-end items-center gap-6 border-t border-border">
            <RequiredNote />
            <SubmitButton label="Update Service" isPending={isPending} />
          </div>
        </form>
      </div>
    </div>
