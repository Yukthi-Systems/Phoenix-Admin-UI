...
const MailboxSpaceUsage = ({ processedData, mailboxesSpace, theme }) => {
  const ChartCard = ({
    title,
    children,
    loading,
  ...
  return (
    <ChartCard
      title="Mailbox Space Usage"
      loading={mailboxesSpace.isLoading}
      error={mailboxesSpace.isError}
      errorMessage="Failed to load mailbox space data"
    >
      {processedData.mailboxSpaceBreakdown.length > 0 ? (
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie
              data={processedData.mailboxSpaceBreakdown}
              cx="50%"
              cy="50%"
              labelLine={false}
              ...
              dataKey="value"
            >
              {processedData.mailboxSpaceBreakdown.map((entry, index) => (
        ...
        <EmptyChart message="No mailbox space data" />
      )}
    </ChartCard>
  );
};

export default MailboxSpaceUsage;