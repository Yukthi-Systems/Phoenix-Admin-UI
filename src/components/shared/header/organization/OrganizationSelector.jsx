...
const OrganizationSelector = ({
  ...
  skipApiLoad = false, // Note: The hook will still fetch in background, but we handle logic safely
}) => {
  ...
  const { uiInfo, updateUiInfo, isLoading: isLoadingUiInfo, isSaving: isSavingUiInfo } = useSyncedUiInfo();
  ...
  // We must preserve other keys in organizationSelector (e.g. sidebar vs dashboard selections)
    ...
    updateUiInfo(
      {
        organizationSelector: {
          ...currentSelectorState, // Preserve other uiInfoKeys
  ...
  const clearInvalidStoredSelection = () => {
    ...
    }, { localOnly: true }); // Usually local clean up is enough
  };
  ...
  useEffect(() => {
    ...
    // 3. BLOCKING: Wait for Profile Org Details
    if (!profileOrgDetails || !profile?.organization_id) return;

    // 4. BLOCKING: Wait for data if loading
    if (isLoading && !data) return;

    // 5. BLOCKING: Wait for the direct by-ID lookup of the stored
    ...
    if (!selectedOrgId) {
      ...
      // Fallback Logic (Default to Parent Org)
    ...
    // Case B: Organization IS selected
    else if (selectedOrgId) {
      if (saveToUiInfo) {
        const stored = getStoredOrgSelection();

        // If nothing stored, or stored ID doesn't match current selection, update storage
  ...
  const ParentOrganizationItem = ({ organization, selectedOrgId, onSelect }) => {
    ...
    return (
      <div
        ...
        className={`flex cursor-pointer items-center gap-3 px-4 py-3 transition-colors ${isSelected
        ...
        <div className="min-w-0 flex-1">
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-3">
              <span
                className={`inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium ${organization.is_active
        ...
        <div className="flex items-center gap-1">
          ...
          {getVisiblePages().map((pageNum) => (
            <button
              ...
              disabled={isLoading}
              className={`rounded px-2 py-1 text-xs font-medium transition-colors ${pageNum === currentPage
