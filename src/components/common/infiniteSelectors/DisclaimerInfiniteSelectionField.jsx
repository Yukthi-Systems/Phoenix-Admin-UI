...
import { getDisclaimerDetails } from "@/api/disclaimer";
...
function DisclaimerDetailsModal({ organizationId, disclaimerId, onClose }) {
  ...
  // Disclaimer details response is nested under `.data`, unlike other entry endpoints.
...
export function DisclaimerInfiniteSelectField({
  control,
  name,
  label,
  errors = {},
  ...
  const fetchOptions = async (pageNum = 1, query = "") => {
    ...
    try {
      ...
      const { disclaimers = [], total_pages = 1 } = response.data?.data || {};

      const newOptions = disclaimers
        // .filter((item) => item.is_active)
        .map((item) => ({
          label: item.disclaimer_name,
          value: item.disclaimer_id,
        }));
