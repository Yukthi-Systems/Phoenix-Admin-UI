...
import { getGeneralPolicyEntry } from "@/api/generalpolicy";
...
function GeneralPolicyDetailsModal({ organizationId, policyId, onClose }) {
  const { formatUserDateNice } = useUserTimezone();

  const { data, isLoading, isError } = useQuery({
    queryKey: ["general_policy_entry", organizationId, policyId],
    queryFn: () => getGeneralPolicyEntry(organizationId, policyId),
    ...
    <PolicyDetailsModal isLoading={isLoading} isError={isError} onClose={onClose}>
      ...
      <InfoCard icon={Info} title="Overview">
        ...
        <InfoItem
          ...
          value={data?.block_all_incoming_emails ? "Blocked" : "Allowed"}
        />
        <InfoItem
          label="Outgoing Emails"
          value={data?.block_all_outgoing_emails ? "Blocked" : "Allowed"}
        />
        <InfoItem
        ...
        />
        <InfoItem
          label="Incoming Exceptions"
          value={`${(data?.incoming_exception_domains?.length || 0) + (data?.incoming_exception_emails?.length || 0)} entries`}
        />
        <InfoItem
          label="Outgoing Exceptions"
          value={`${(data?.outgoing_exception_domains?.length || 0) + (data?.outgoing_exception_emails?.length || 0)} entries`}
        />
        {data?.created_at && (
          <InfoItem
            label="Created"
...
export function GeneralPolicyInfiniteSelectionField({
  control,
  name,
  label,
  errors = {},
