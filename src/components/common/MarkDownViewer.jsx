  ...
  const renderMarkdown = (rawText) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmedLine.match(/^#{1,6}\s/)) {
        ...
        if (level === 1) {
          processedLines.push(
            `<h1 class="text-2xl font-bold mt-4 mb-2 text-foreground">${text}</h1>`,
          );
        } else if (level === 2) {
          processedLines.push(
            `<h2 class="text-xl font-bold mt-3 mb-2 text-foreground">${text}</h2>`,
          );
        } else if (level === 3) {
          processedLines.push(
            `<h3 class="text-lg font-semibold mt-3 mb-2 text-foreground">${text}</h3>`,
          );
        } else {
          processedLines.push(
            `<h4 class="text-base font-semibold mt-2 mb-1 text-foreground">${text}</h4>`,
          );
        }
      }
      ...
      // Lists
        ...
        listItems.push(
          `<li class="ml-5 list-disc marker:text-primary text-foreground">${itemText}</li>`,
        );
      }
      // Numbered lists
        ...
        if (listItems.length === 0 || inList) {
          listItems.push(
            `<li class="ml-5 list-decimal marker:text-primary text-foreground">${itemText}</li>`,
          );
          inList = true;
        }
      }
      // Code blocks (```)
        ...
        processedLines.push(
          `<pre class="bg-muted p-3 rounded my-2 overflow-x-auto"><code class="text-sm font-mono text-foreground">${code}</code></pre>`,
        );
      }
      // Empty lines
      else if (!trimmedLine) {
        ...
        if (
          ...
          !processedLines[processedLines.length - 1].includes("h-3")
        ) {
      ...
      // Regular text
        ...
        processedLines.push(
          `<p class="my-1.5 leading-relaxed text-foreground">${escapedLine}</p>`,
        );
      }
    }
    ...
    html = html.replace(
      /\*\*\*(.+?)\*\*\*/g,
      '<strong class="font-bold italic text-foreground">$1</strong>',
    );
    html = html.replace(
      /___(.+?)___/g,
      '<strong class="font-bold italic text-foreground">$1</strong>',
